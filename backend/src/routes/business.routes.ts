...
businessRoutes.post('/join', async (req, res) => {
...
businessRoutes.get('/mine', async (req, res) => {
  const businesses = await businessService.listForUser(req.user!.id);
  res.json(businesses);
});

businessRoutes.get('/:businessId', requireMembership, async (req, res) => {
  const business = await businessService.getById(req.params.businessId);
  res.json(business);
});

businessRoutes.delete('/:businessId', requireOwner, async (req, res) => {
  await businessService.delete(req.params.businessId);
  res.status(204).send();
});

businessRoutes.get('/:businessId/members', requireMembership, async (req, res) => {
  const members = await businessService.listMembers(req.params.businessId);
  res.json(members);
});

businessRoutes.delete('/:businessId/members/:userId', requireOwner, async (req, res) => {
