...
inventoryRoutes.get('/', requireMembership, async (req, res) => {
  const items = await inventoryService.list(req.params.businessId);
  res.json(items);
});

inventoryRoutes.post('/', requireMembership, async (req, res) => {
...
inventoryRoutes.put('/:itemId', requireMembership, async (req, res) => {
  const { name, price, quantity, unit } = req.body;
  if (!name?.trim() || price == null || quantity == null || !unit?.trim()) {
    return res.status(400).json({ error: 'name, price, quantity, unit are required' });
  }
...
inventoryRoutes.delete('/:itemId', requireOwner, async (req, res) => {
